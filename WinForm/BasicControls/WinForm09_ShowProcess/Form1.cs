...
namespace WinForm09_ShowProcess
{
    public partial class Form1 : Form
    {
        ...
        private void TotalView(int m, int n)
        {
            ...
            this.tsslMemory.Text = "실제 메모리 : " + n.ToString() + " %";
        }
        ...
        private void ProccessKill()
        {
            try
            {
                int PID = Convert.ToInt32(this.listView1.SelectedItems[0].SubItems[1].Text);
                Process tProcess = Process.GetProcessById(PID);
                if (!(tProcess == null))
                {
                    var dlr = MessageBox.Show(this.listView1.SelectedItems[0].SubItems[0].Text +

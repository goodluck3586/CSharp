...
namespace GenericConstraint
{
    ...
    class BaseArray<U> where U : Base  // U는 명시한 기반 클래스이거나 기반 클래스의 파생 클래스여야 한다.
    {
        ...
        {
            source.CopyTo(Array, 0); // 현재 1차원 배열의 모든 요소를 지정된 대상 배열 인덱스부터 시작하여 지정된 1차원 배열에 복사
        }
    }
    ...
    class Program
    {
        ...
        private static T CreateInstance<T>() where T : new()  // T는 매개변수가 없는 기본 생성자가 정의되어 있어야 한다.
        {
            return new T();
        }
    }
